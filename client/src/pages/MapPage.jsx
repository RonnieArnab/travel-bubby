...
import { DESTINATIONS, EXAMPLE_PLACES } from "../lib/destinations.js";
...
import { enqueueWalkPoint, drainQueue } from "../lib/offlineQueue.js";
import {
...
function FitBounds({ points, focusId, deps }) {
  ...
  useEffect(() => {
    ...
    const mobile = map.getSize().x < 500;
    map.fitBounds(points, {
      paddingTopLeft: mobile ? [35, 90] : [350, 90],
      paddingBottomRight: mobile ? [55, map.getSize().y * 0.43] : [75, 55],
      maxZoom: 15,
    });
...
export function MapPage() {
  const { places, refresh, loading, error } = usePlaces();
  const nav = useNavigate();
  const [params, setParams] = useSearchParams();
  const requestedMode = params.get("mode");
  const mode = VALID_MODES.has(requestedMode) ? requestedMode : "browse";
  const destination =
    mode === "browse"
      ? DESTINATIONS.find((d) => d.id === params.get("destination"))
      : null;
  const collectionId = mode === "browse" ? params.get("collection") : null;
  const displayPlaces = useMemo(
    () =>
      destination
        ? EXAMPLE_PLACES[destination.id]
        : collectionId
          ? places.filter((p) => String(p.collection_id) === collectionId)
          : places,
    [destination, collectionId, places],
  );
  ...
  useEffect(() => {
    if (showHeat && !heatPoints.length) {
      api
        .heatmapPoints()
        .then((d) => setHeatPoints(d.map((p) => [p.lat, p.lng])))
        .catch(() => {});
    }
  ...
  const mapped = useMemo(
    () => displayPlaces.filter((p) => p.lat != null && p.lng != null),
    [displayPlaces],
  );
  useEffect(() => {
    const placeId = params.get("place");
    const place = mapped.find((p) => String(p.id) === placeId);
    setFocused(place ? { id: place.id, lat: place.lat, lng: place.lng } : null);
  }, [params, mapped]);
  ...
  const totalMeters = useMemo(() => {
    ...
    for (let i = 1; i < walkPoints.length; i++)
      d += distanceMeters(walkPoints[i - 1], walkPoints[i]);
    return d;
  }, [walkPoints]);
  ...
  async function logVisit(p) {
    ...
    try {
      ...
      await refresh();
      setToast({ kind: "ok", title: `Visit saved for ${p.name}` });
    } catch (e) {
      setToast({
        kind: "warn",
  ...
  async function savePreviewPlace(p) {
    setSaving(p.id);
    try {
      ...
      await refresh();
      setToast({ kind: "ok", title: `${p.name} is in your collection` });
    } catch (e) {
      setToast({
        kind: "warn",
  ...
  const startWalk = useCallback(async () => {
    setWalkBusy(true);
    try {
      const w = await api.startWalk({
        name: walkName.trim() || null,
        radius: walkRadius,
      });
      setWalk(w);
      setWalkPoints([]);
      ...
      const id = navigator.geolocation.watchPosition(
        ...
        (err) =>
          setToast({ kind: "warn", title: "GPS error", body: err.message }),
  ...
  const stopWalk = useCallback(async () => {
    ...
    try {
      ...
      setToast({
        kind: "ok",
        title: "Walk ended",
        body: `${autoVisits.length} place(s) marked visited.`,
      });
    } catch (err) {
  ...
  const walkPolyline = useMemo(
    () => walkPoints.map((p) => [p.lat, p.lng]),
    [walkPoints],
  );
    ...
    <div className="page full map-shell">
      ...
      >
          ...
          >
            <Popup>
              <div style={{ minWidth: 220 }}>
                ...
                <div
                  style={{ fontSize: 13, marginBottom: 12, fontWeight: 500 }}
                >
                  {(p.visit_count ?? 0) > 0
                ...
                {p.notes && (
                  <p className="popup-notes">{p.notes.slice(0, 220)}</p>
                )}
                {destination ? (
                  <button
                    className="sm"
                    disabled={
                      saving === p.id ||
                      places.some(
                        (saved) => saved.name === p.name && saved.lat === p.lat,
                      )
                    }
                    onClick={() => savePreviewPlace(p)}
                  >
                    {places.some(
                      (saved) => saved.name === p.name && saved.lat === p.lat,
                    )
                  ...
                  <button
                    ...
                    onClick={() => logVisit(p)}
                  >
                    {saving === p.id ? "Saving…" : "I’ve been here"}
                  </button>
                )}
              </div>
            </Popup>
        ...
        {walkPolyline.length > 1 && (
          <Polyline
            positions={walkPolyline}
            pathOptions={{ color: "var(--ink)", weight: 4, opacity: 0.85 }}
          />
        )}
        {mode === "walk" &&
          walkPoints
        ...
        <FitBounds
          points={mapped.map((p) => [p.lat, p.lng])}
          focusId={focused?.id}
          deps={[mode, mapped, mapStyle]}
        />
      ...
      <div className="map-mode-position">
        <MapModeBar mode={mode} onChange={setMode} />
      </div>

      {/* Side panel — content swaps per mode, shell stays */}
      <div className={`map-panel mode-${mode}`}>
        {collectionId && (
          <div className="map-preview-label">
            <div>
              <strong>
                {displayPlaces[0]?.collection_name || "Reel collection"}
              </strong>
              <small style={{ display: "block" }}>
                {mapped.length} mapped · {displayPlaces.length - mapped.length}{" "}
                awaiting a location
              </small>
            </div>
            <button onClick={() => setMode("browse")}>All places</button>
          </div>
        )}
        {destination && (
          <div className="map-preview-label">
            <span>Destination preview · not your saved places</span>
            <button onClick={() => setMode("browse")}>Exit</button>
          </div>
        )}
        {mode === "browse" && (
          <BrowsePanel
      ...
      <div className="fab-stack">
        ...
        <button
          className="fab icon-only accent"
          onClick={() => nav("/add")}
          title="Add a place"
      ...
      <div className="map-legend">
        ...
        <span>
          <i className="visited-dot" /> Been there
        </span>
      </div>
